;(function($){
    ...
    function Calendar (options) {
        ...
        Calendar.prototype.init = function () {
            // 未来七天
            var weeksOfzhTW = ['CN', 'Hai', 'Ba', 'Tư', 'Năm', 'Sáu', 'Bảy']
            var today = new Date();
        ...
        Calendar.prototype.minuteClickHandler = function () {
            ...
            $('.calendar-7-minute').bind('click', function () {
                ...
                var time = that.day + '-' + that.month + '-' + that.year + ' ' + that.hour + ':' + that.minute

                if (!$(this).hasClass('disabled')) {
                    that.trigger.val(time);
                    that.callback(new Date(that.year, that.month, that.day, that.hour, that.minute, 0, 0));
                    $(that.calendarSelector).remove();
                }
