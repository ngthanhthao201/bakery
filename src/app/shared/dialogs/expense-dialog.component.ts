...
export class ExpenseDialog {
  ...
  filteredMaterialBuyingHistories: any;
  currencyMaskOptions ={prefix: '' , suffix: 'đ ', thousands: '.', precision:0 };
  myControl: FormControl = new FormControl();

  constructor(
    public dialogRef: MatDialogRef<ExpenseDialog>, private apiService: ApiService, private snotifyService: SnotifyService,
    private shopService: ShopService, private printService: PrintService,
  ...
  filterMaterialBuyingHistories(val: string): any[] {
   
    if(!val || typeof(val) != 'string'){
      return this.expenseHeader.materialProvider.materialBuyingHistories;
    }
    return this.expenseHeader.materialProvider.materialBuyingHistories.filter(mp =>
      mp.material && mp.material.name.toLowerCase().indexOf(val.toLowerCase()) > -1);
  }
  ...
  addTransaction() {
    ...
    setTimeout(()=>{
      $('#material-name-' + (this.expenseHeader.expenseTransactions.length-1)).focus();
    } ,250)
  }
      ...
      buttons: [
        {
            text: "Ok",
            action: () => {
              this.apiService.removeExpenseTransaction(expenseTransactionId).subscribe(_r=>{
  ...
  materialChanged(history, material) {
   ...
   this.filteredMaterialBuyingHistories = this.filterMaterialBuyingHistories(material);
  ...
  save(){
    _.each(this.expenseHeader.expenseTransactions, (t) => {
      ...
      if(t.materialBuyingHistory){
        delete t.materialBuyingHistory.materialProvider
        if(typeof(t.materialBuyingHistory.material) == 'string'){
          t.materialBuyingHistory.material = {
            name: t.materialBuyingHistory.material,
